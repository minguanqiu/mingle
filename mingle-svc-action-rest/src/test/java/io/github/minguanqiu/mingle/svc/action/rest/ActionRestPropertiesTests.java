...
public class ActionRestPropertiesTests {
    ...
    @Test
    void testProperties() {
        RestActionProperties.RestProperties.ServerProperties serverProperties = properties.getRest().getServer().get("Simple");
        ...
        RestActionProperties.RestProperties.MockProperties mockProperties = properties.getRest().getMock().get("SimpleAction");
        assertThat(mockProperties.getCode()).isEqualTo(400);
        assertThat(mockProperties.getHeader()).contains(Map.entry("test","test"),Map.entry("test1","test1"));
        assertThat(mockProperties.getResponseBody().getMediaType()).isEqualTo("application/xml");
        assertThat(mockProperties.getResponseBody().getContent()).isEqualTo("{\"test\":\"test\",\"test1\":\"test1\"}");
